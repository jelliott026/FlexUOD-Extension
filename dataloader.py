...
def dataLoader(setIndex, featureType="ResNet"):
    ...
    if setIndex == 0:
        ...
        if featureType == "ResNet":
            ...
            train_data = np.load(path + "train_resNet50.npy")
            train_gt = np.load(path + "train_gt.npy")
            test_data = np.load(path + "test_resNet50.npy")
        ...
        if featureType == "ResNet":
            path = os.path.join(ResNet_path, "dogvscat/cats_vs_dogs224feats/")
...
def load_data_with_outliers(all_feats, all_labels, ind, p):
    np.random.seed(42)
